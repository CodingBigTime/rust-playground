...
fn main() {
    App::new()
        .insert_resource(WindowDescriptor {
            title: "Coding Big Time - Cursor Squares".to_string(),
            width: 800.,
            height: 600.,
            ..Default::default()
        })
...
fn move_mouse(
    ...
    if let Some(cursor_position) = window.cursor_position() {
        mouse_squares.last_mut().unwrap().0.translation =
            window_to_world(cursor_position, window, camera_transform.single());
    }
}
